...
def test_Training():
    ...
    for name, value in {
        ...
        assert hasattr(main.Training, name), (
            f'У класса `Training` должна быть константа `{name}`'
            f'{value.description}.'
        )
        assert getattr(main.Training, name) == value.value, (
            f'Значение константы `{name}` в классе `Training` должно быть '
            f'равно `{value.value}`.'
        )
    MIN_IN_H = 60
    assert MIN_IN_H in main.Training.__dict__.values(), (
...
def test_Swimming():
    ...
    Swimming_attr_values = main.Swimming.__dict__.values()
    for value_info in {
...
def test_SportsWalking():
    ...
    SportsWalking_attr_values = main.SportsWalking.__dict__.values()
    for value_info in {
            'CALORIES_WEIGHT_MULTIPLIER': ValueInfo(
                0.035,
...
def test_Running():
    ...
    assert inspect.isclass(main.Running), '`Running` должен быть классом.'
    assert issubclass(main.Running, main.Training), (
        'Класс `Running` должен наследоваться от класса `Training`.'
    )
    Running_attr_values = main.Running.__dict__.values()
    for value_info in {
...
def test_main():
    ...
    assert callable(main.main), '`main` должна быть функцией.'
